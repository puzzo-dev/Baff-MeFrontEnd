...
import { getBlogPosts } from '../lib/erpnext';
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      try {
        ...
        if (params?.slug) {
          // Single post view
          const post = erpPosts?.find(p => p.slug === params.slug); // Assuming getBlogPosts returns an array of posts
          if (post) {
            setCurrentPost(post);
          }
        } else {
          // Posts listing
          setPosts(erpPosts || []); // Handle case where data is not yet loaded

          // Extract unique categories
          const uniqueCategories = Array.from(
            new Set((erpPosts || []).map(post => post.category))
          );
